...
if not vim.loop.fs_stat(lazypath) then
    ...
    require("lazy").setup({
        ...
        -- Snacks.nvim (Dashboard + lots of UI goodies)
        {
            "folke/snacks.nvim",
            priority = 900, -- after colorscheme but early
            lazy = false,
            opts = {
                picker = { enabled = true },
                dashboard = {
...
-- Telescope: live_grep_args (ripgrep power search)
{
...
-- Session persistence (used by Snacks dashboard "Restore Session")
{
...
map("n", "<leader>fG", "<cmd>lua require('telescope').extensions.live_grep_args.live_grep_args()<cr>", { desc = "Live grep (args)" })
...
pcall(telescope.load_extension, "live_grep_args")
    ...
    callback = function()
        if vim.fn.argc() == 0 then
            -- Show dashboard when starting without files
            pcall(function()
